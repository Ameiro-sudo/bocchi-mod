 ...
 * 即可检测哪些常量已经漂移。Node 直读求值见 sync/facts-dump.mjs。
 *
 * 注意: sync/check-layout.py 的正则解析依赖本文件的缩进形状
 * (组头 4 空格缩进, fact 行 6 空格缩进), 调整格式时须同步修改解析器。
 * ==========================================================================*/
 ...
 * 求值帧 (画布尺寸) 可选: 预览用 1280x720 (默认), 漂移检查用 Java 端逻辑画布
 * 480x270 —— 同一组公式在两帧下成比例, 与 check-layout.py 的参考系保持一致。 */
const DEFAULT_FRAME = Object.freeze({ w: W, h: H });
const cacheByFrame = new Map();
const frameCache = (frame) => {
  const key = `${frame.w}x${frame.h}`;
  let m = cacheByFrame.get(key);
  if (!m) { m = {}; cacheByFrame.set(key, m); }
  return m;
};
...
function evalFact(name, frame, stack) {
  const cache = frameCache(frame);
  ...
  const bind = {
    scaledWidth: frame.w, scaledHeight: frame.h,
    W: frame.w, H: frame.h, width: frame.w, height: frame.h,
  };
  for (const id of refsOf(fact.expr)) {
    // 引用另一组的 fact (如 splash.logoX -> poulsen.rect1X)
    const other = G[group][id];
    if (other) bind[id] = evalFact(group + "." + id, frame, stack.concat([name]));
    else if (G.poulsen[id]) bind[id] = evalFact("poulsen." + id, frame, stack.concat([name]));
...
/** 默认帧 (预览 1280x720) 求值 */
export function value(name, stack) { return evalFact(name, DEFAULT_FRAME, stack); }
/** 指定帧求值 (漂移检查用 Java 逻辑画布帧) */
export function valueIn(name, w, h, stack) { return evalFact(name, { w, h }, stack); }

/** 全部 fact 求值结果 (供 sync/facts-dump.mjs 与测试使用) */
export function all() {
  const out = {};
...
export function clearCache() { cacheByFrame.clear(); }
