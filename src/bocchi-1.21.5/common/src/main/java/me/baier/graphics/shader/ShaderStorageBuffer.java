...
public class ShaderStorageBuffer implements AutoCloseable, MinecraftRenderInstance {
    private int buffer;
    ...
    public void bindRange(int index, long offset, long size) {
        glBindBufferRange(GL_SHADER_STORAGE_BUFFER, index, getBufferID(), offset, size);
    }
    ...
    public void allocate(long size) {
        if (dsa()) {
            ...
            nglBufferData(GL_SHADER_STORAGE_BUFFER, size, 0, GL_DYNAMIC_DRAW);
        }
    }
    ...
    public void upload(long offset, long size, long data) {
        if (dsa()) {
            ...
            glBindBuffer(GL_SHADER_STORAGE_BUFFER, getBufferID());
            nglBufferSubData(GL_SHADER_STORAGE_BUFFER, offset, size, data);
        }
    }
